...
### P0-001 — 发布 pip 包
- **状态**: ✅ 已完成
- **创建**: 2026-07-28 | **完成**: 2026-08-25
- **描述**: `pip install tianshu-agent` 一键安装（import 名仍为 `tianshu`）。原 `tianshu` 包名被占用，定名 `tianshu-agent`，发布 0.2.0：https://pypi.org/project/tianshu-agent/
- **备注**: 本机 GBK 控制台炸 twine 进度条 → 上传须 `PYTHONIOENCODING=utf-8`

### P0-002 — 清理死代码 + 项目结构规范
- **状态**: ✅ 已完成
- **创建**: 2026-07-28 | **完成**: 2026-08-11
- **描述**: ~~`main.py` 中 `_main_deprecated()` 300+ 行死代码~~ 已随 08-11「死代码清理：删除 5 个死模块 ~800 行」完成（见 §七 表格）；`__init__.py` 导出 8/4 补齐
- **预计**: ~~2 小时~~ 已完成
...
**完成率: 62%** (16/26)，另有 3 项进行中（含 Android MC0-MC3 完成待真机）
...
```
已完成:     P0-001(pip包 tianshu-agent 0.2.0) ✅ → P0-002(死代码清理) ✅ → P1-004(MCP) ✅ → P2-001(RAG) ✅ → P2-006(Agent间通信) ✅
当前主线:   TS-018 手机助手 MC0-MC3 ✅ → 真机验证 → MC4 视觉(TS-019)
外围阻塞:   TS-014 服务器推送（等腾讯云恢复）
待激活:     P1-001(飞书Bot) → P2-003(语音) → P2-004(系统托盘) → P3-001(系统控制) → P3-004(文档站) → P3-005(性能) → P3-007(Skills市场)
```
