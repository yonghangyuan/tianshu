...
1. **三爻架构**: 天(审计)·人(意图)·地(执行)——每个决策经过三层
2. **自有浏览器**: httpx快速抓取 + Edge CDP渲染JS页面。零外部下载
3. **自有搜索**: cn.bing.com → 搜狗 → 百度。零 API Key
4. **审计不是后装**: decision_id 从 AgentCore 第一行就埋入
5. **代码量控制**: 12K行·一人可全审。不追功能数量，追可信度
