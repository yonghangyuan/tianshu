...
# ── 旧 _main() 已替换为 _main_sync() ──

async def _main_deprecated() -> None:
    """旧入口——已被 _main_sync() 替代。"""
    from tianshu.core.config import load_providers, load_routing_config
    from tianshu.core.service import AgentCore
    from tianshu.sdk.models import AgentContext, AgentRequest
    from tianshu.core.setup import (
        any_key_configured, check_keys, run_setup_wizard,
    )
    from tianshu.core.splash import render as render_splash
    ...
    if not providers_yaml.exists():
        print("错误: config/providers.yaml 不存在")
        print("请复制 config/providers.yaml.example 并填入你的 API Key")
        return
    ...
    if configured == 0:
        ...
        _rich_console.print(Text("  ⚠ 未配置 API Key，输入 --setup 开始配置", style="dim yellow"))
    ...
    if args:
        ...
        if user_input in ("--audit", "/audit"):
            ...
            _show_models(core._registry, core.router)
            return
        elif user_input in ("--setup", "/setup", "setup"):
            keys = run_setup_wizard()
            if keys:
                save_user_keys(keys)
                # 重新加载 provider
                registry = load_providers(providers_yaml, extra_keys=load_user_keys())
                print(f"\n✅ 已保存 {len(keys)} 个 Key，模型已就绪。")
            return
        else:
            # 单次执行（流式）
    ...
    if not any_key_configured():
        ...
        if response not in ("n", "no"):
            ...
            if keys:
                ...
                core.setup(registry=registry, routing=routing, system_prompt=system_prompt, db_path=str(_project_root / "tianshu.db"), skill_discover=True)
                print(f"\n  ✅ 已保存 {len(keys)} 个 Key。输入 '你好' 开始对话。\n")
            else:
                print("\n  已跳过。可稍后使用 --setup 配置。\n")
        else:
            print("  已跳过。可稍后使用 --setup 配置。\n")
    ...
    _current_session_title = ""

    # 尝试恢复最近会话
    recent_sessions = session_store.list_sessions(limit=1)
    if recent_sessions:
        _rich_console.print(
            f"  [dim]最近会话: {recent_sessions[0]['title'][:40]}"
            f"  [/dim][dim](输入 /session resume 恢复)[/dim]"
        )
    ...
    from tianshu.gateway.cli import StreamRenderer, chat_once
    from tianshu.core.commands import get_registry, CommandContext
    ...
    def _prompt_text() -> str:
        ...
        model_part = f"[cyan]{m}[/cyan]" if m else "天枢"
        mode_part = _MODE_LABELS.get(_mode, "")
        return f"{mode_part} {model_part}> "
    ...
    while True:
        ...
        if user_input in ("exit", "quit", "q", "/exit"):
            ...
            if keys:
                ...
                choice = input("  是否选择默认模型？[Y/n] ").strip().lower()
                if choice not in ("n", "no"):
                    ...
                    if idx is not None:
                        ...
                        _rich_console.print(f"  ✅ 默认模型: {full}\n")
            continue
            ...
            if rc:
                ...
                _rich_console.print(_Panel(rc[:3000], title="Thinking", border_style="dim"))
            else:
            ...
            continue
        elif user_input in ("automode", "/automode", "mode", "/mode"):
            new_mode = _cycle_mode()
            core._automode = (_mode == "auto")
            _rich_console.print(
                f"  {_MODE_LABELS[_mode]}  "
                "[dim](Shift+Tab 或 /mode 切换)[/dim]\n"
            )
            continue
        ...
        try:
            ...
            from tianshu.sdk.models import ToolCallConfirm
            ...
            # 自动保存会话
            _current_session_title = user_input[:50]
            session_store.save(ctx, title=_current_session_title)
        except Exception as e:
